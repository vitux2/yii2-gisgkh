...
def parse_args():
    ...
    parser.add_argument('xades', help='Xades template file')
...
def get_element(tree, el_path, namespaces):
    ...
    Поиск элемента
    """
    elements = tree.xpath(el_path, namespaces=namespaces)
...
def main():
    ...
    xades_template_file = args.xades
    ...
    sign_data = dict(
        ...
        signing_time=datetime.now(tz=pytz.timezone('Asia/Yekaterinburg')).replace(microsecond=0).isoformat(),
        x509_issuer_name=get_issuer(key_file),
        x509_sn=get_serial(key_file)
    )

    # элемент для подписи
    signing_el = get_element(tree, '//*[@Id]', nsmap)

    if signing_el is None:
      return get_canonic(tree)

    sign_data['signed_id'] = signing_el.attrib['Id']
